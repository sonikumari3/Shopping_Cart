...
const updateProfile = async (req,res)=>{
    try{
       ...
       let updateData = await user.findOneAndUpdate({_id : req.params.userId},{$set : {...data}, updateAt : Date.now()}, {new : true, upsert : true})

       return res.status(200).send({status : true, message : 'User Profile updated successfully', data : updateData})
